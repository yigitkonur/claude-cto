...
class ErrorHandler:
    ...
    def handle_error(
        ...
        if isinstance(error, ProcessError):
            ...
            error_info.update({
                ...
                "debugging": {
                    "line_preview": error.line[:100] + "..." if hasattr(error, 'line') and len(error.line) > 100 else error.line if hasattr(error, 'line') else None,
                    "json_error_type": type(error.original_error).__name__ if hasattr(error, 'original_error') else None
                }
            })
            ...
            error_info.update({
                ...
                "debugging": {
                    ...
                    "data_keys": list(error.data.keys()) if hasattr(error, 'data') and isinstance(error.data, dict) else None
                }
            })
    ...
    def _get_recovery_suggestions(cls, error: Exception) -> list:
        ...
        return ["Check task logs for details", "Retry the task", "Contact support if issue persists"]
