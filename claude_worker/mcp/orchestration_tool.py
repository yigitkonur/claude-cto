...
async def create_mcp_orchestration(
    ...
    for task in tasks:
        if not all(k in task for k in ["identifier", "execution_prompt", "working_directory", "system_prompt"]):
    ...
    async with httpx.AsyncClient() as client:
        response = await client.post(
            f"{server_url}/api/v1/orchestrations",
...
async def get_mcp_orchestration_status(orchestration_id: int) -> Dict[str, Any]:
    ...
    async with httpx.AsyncClient() as client:
        response = await client.get(
            f"{server_url}/api/v1/orchestrations/{orchestration_id}",
            timeout=10.0
        )
        response.raise_for_status()
